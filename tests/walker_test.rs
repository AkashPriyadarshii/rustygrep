...
fn finds_all_source_files() {
    let dir = setup_repo();
    let walker = FileWalker::new(vec![dir.path().to_path_buf()], false, false, false, None, None, 0);
    let files = walker.walk();

    assert!(files.iter().any(|p| p.to_string_lossy().contains("main.rs")));
    assert!(files.iter().any(|p| p.to_string_lossy().contains("lib.rs")));
    assert!(files.iter().any(|p| p.to_string_lossy().contains("config.rs")));
}
...
fn respects_gitignore() {
    ...
    assert!(!files.iter().any(|p| p.to_string_lossy().contains("test.log")));
}
...
fn file_type_filter() {
    ...
    let walker = FileWalker::new(vec![root.to_path_buf()], false, false, false, Some("py".into()), None, 0);
    let files = walker.walk();

    assert!(files.iter().any(|p| p.to_string_lossy().contains("app.py")));
    assert!(!files.iter().any(|p| p.to_string_lossy().contains("style.css")));
}
...
fn file_type_not_filter() {
    ...
    let walker = FileWalker::new(vec![root.to_path_buf()], false, false, false, None, Some("py".into()), 0);
    let files = walker.walk();
